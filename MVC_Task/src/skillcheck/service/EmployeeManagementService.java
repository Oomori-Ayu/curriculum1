    ...
    private ResponseBean executeDBAccess(final ExecuteCase eCase, final List<EmployeeBean> pEmployeeBeanList) throws MVCException {
        ...
        try {
            ...
            while (this.resultSet.next()) {
                ...
                //String tmpempId  = this.resultSet.getString(meta.getColumnName(1));
                //String tmppassword  = this.resultSet.getString(meta.getColumnName(2));
                //String tmpname  = this.resultSet.getString(meta.getColumnName(3));
                //String tmpmail  = this.resultSet.getString(meta.getColumnName(4));
                //String tmpLanguage  = this.resultSet.getString(meta.getColumnName(5));
                //String tmpcomment  = this.resultSet.getString(meta.getColumnName(6));

                //EmployeeBean employeeBean = new EmployeeBean();
                //employeeBean.setEmpId(tmpempId);
                //employeeBean.setPassword(tmppassword);
                ///employeeBean.setName(tmpname);
                //employeeBean.setMail(tmpmail);
                //employeeBean.setProgramingLanguage(tmpLanguage);
                ///employeeBean.setComment(tmpcomment);

                EmployeeBean employeeBean = new EmployeeBean(
                		this.resultSet.getString(meta.getColumnName(1)),
                		this.resultSet.getString(meta.getColumnName(2)),
                		this.resultSet.getString(meta.getColumnName(3)),
                		this.resultSet.getString(meta.getColumnName(4)),
                		this.resultSet.getString(meta.getColumnName(5)),
                		this.resultSet.getString(meta.getColumnName(6)));
            ...
            for(int i=0 ; i<empResultList.size() ; i++) {
            	Logger.log(new Throwable(), i + " = " + empResultList.get(i));
        ...
        try {
            ...
            case FIND_BY_EMPID:
                ...
                if (Objects.nonNull(emp)) {
                    ...
                    String sb_Query = sbQuery.toString();

                    /*
                    * 任意のユーザーを検索できるように、プリペアドステートメントを記述。
                    */
                    preparedStatement = connection.prepareStatement(ConstSQL.SQL_SELECT);
