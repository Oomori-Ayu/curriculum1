...
public abstract class BaseServlet extends HttpServlet {
    ...
    //protected static final String CONST_ELEMENT_NAME_REQUEST = "empId";
    ...
    protected boolean validateLogin(final HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            // ←ここへ記述
        	///ConstSQL cs = new ConstSQL();
        	//ems.getEmployeeData(ExecuteCase.FIND_BY_EMPID,cs.search(reqEmpId,reqPassword));
        	ems.getEmployeeData(ExecuteCase.FIND_BY_EMPID,new EmployeeBean(reqEmpId));

        	 if (Objects.nonNull(responseBean.getEmplyeeBeanList())) {
        		 Logger.log(new Throwable(), "ok");

        	 }else {
        			 Logger.log(new Throwable(), "ng");
        		 }


            // 最初の1件を取得
            resEmployeeBean = responseBean.getEmplyeeBeanList().stream().findFirst().orElse(null);
            Logger.log(new Throwable(), "終了２");

            if (Objects.nonNull(resEmployeeBean)) {
                // パスワードチェック
            	Logger.log(new Throwable(), "終了３");
                final String hashPassword = PasswordHashUtil.getSafetyPassword(reqPassword, reqEmpId);
                Logger.log(new Throwable(), "終了４");
            ...
            Logger.log(e);
            this.responseBean = ((MVCException) e).getResponseBean();
        } finally {
